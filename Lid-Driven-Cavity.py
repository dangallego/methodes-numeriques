...
def cdf_x(funct):
    ...
    dfdx[1:-1, 1:-1] = (funct[1:-1, 2:] - funct[1:-1, 0:-2]) / (2 * element_length) # for 2D array: [row, column] == [y,x] , which can be confusing but means we hold y constant 
    return dfdx
...
def cdf_y(funct):
    ...
    # Operate only on interior points to avoid boundary issues
    dfdy[1:-1, 1:-1] = (funct[2:, 1:-1] - funct[0:-2, 1:-1]) / (2 * element_length)
    return dfdy
...
def laplacian(funct):
    ...
    laplacian[1:-1, 1:-1] = (funct[1:-1, 0:-2] + funct[0:-2, 1:-1] + funct[1:-1, 2:] + funct[2:, 1:-1] - 4 * funct[1:-1, 1:-1]) / (element_length**2)
    return laplacian
